...
@interface BWTSodeScanVC ()<TZImagePickerControllerDelegate>
...
- (void)setupScanManager {
    ...
    [_scanManager scanResult:^(NSArray *metadataObjects) {
        if (metadataObjects != nil && metadataObjects.count > 0) {
            ...
            [self createCouponWithStr:url];
        } else {
            [MSTipView showWithMessage:@"暂未识别出二维码"];
...
- (UIView *)topContainer {
    if (!_topContainer) {
        _topContainer = [[UIView alloc] initWithFrame:CGRectMake(0, 0, Device_Width, Device_Height-100-NAVH)];
...
- (QRScanBottomView *)bottomBtnView {
    if (!_bottomBtnView) {
        _bottomBtnView = [[QRScanBottomView alloc] initWithFrame:CGRectMake(0, Device_Height-kNavigationBarHeight - 100, Device_Width, 100) showType:ShowTypeAlbum];
...
- (void)albumBtnClick {
    ...
    [pickerController setDidFinishPickingPhotosHandle:^(NSArray<UIImage *> *photos, NSArray *assets, BOOL isSelectOriginalPhoto) {
        ...
        if (features.count == 0) {
            ...
            [self createCouponWithStr:resultStr];
...
- (void)createCouponWithStr:(NSString *)str{

    NSString *couponIDStr;
    NSString *moduleStr;
    NSArray *subStr1 = [str componentsSeparatedByString:@"?"];
    if (subStr1.count >= 2) {
        NSArray *subStr2 = [subStr1[1] componentsSeparatedByString:@"&"];
        for (NSString *paramStr in subStr2) {
            if ([paramStr hasPrefix:@"module"]) {
                moduleStr = [paramStr substringFromIndex:7];
                
            }
            if ([paramStr hasPrefix:@"couponid"]) {
                couponIDStr = [paramStr substringFromIndex:9];
            }
        }
    }
    
    
    if ([moduleStr isEqualToString:@"coupon"] && couponIDStr) {
        NSString *paramStr = couponIDStr;
        NSMutableDictionary *param = [[NSMutableDictionary alloc] init];
        [param setValue:BWTUsertoken forKey:@"token"];
        [param setValue:[NSNumber numberWithInteger:BWTUserID] forKey:@"userId"];
        [param setValue:[NSNumber numberWithInteger:[paramStr integerValue]] forKey:@"couponId"];
        ...
        [AFNetworkTool postJSONWithUrl:Coupon_get parameters:param success:^(id responseObject) {
            ...
            if (responseObject) {
                ...
                [MSTipView showWithMessage:@"优惠劵领取成功"];
                if (self.getCouponSuccessBlock) {
                    self.getCouponSuccessBlock();
                }
                [self.navigationController popViewControllerAnimated:YES];
            }
            
            
            // :pages/index/index?couponid=1&source=qrcode&module=coupon
