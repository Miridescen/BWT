...
@interface BWTOrderVC ()<UITableViewDelegate, UITableViewDataSource, UITextViewDelegate>
...
- (void)viewDidLoad {
    ...
    for (OrderCellModel *model in self.cellModelArr) {
        _xiaoji += (CGFloat)(model.goodNum * model.price);
        _totalItemCount += model.goodNum;
    }
...
- (void)loadCouponData{
    ...
    [param1 setValue:[NSNumber numberWithInteger:0] forKey:@"category"];
...
- (void)clearBtnClick:(UIButton *)btn{
    ...
    self.remark = self.message.text;
    NSMutableArray *orderArr = [@[] mutableCopy];
    for (OrderCellModel *model in self.cellModelArr) {
        NSMutableDictionary *orderDic = [[NSMutableDictionary alloc] initWithCapacity:7];
        [orderDic setObject:model.orderID forKey:@"orderId"];
        if (self.selectCoupon) {
            [orderDic setObject:[NSNumber numberWithInteger:self.selectCoupon._id] forKey:@"couponId"];
            [orderDic setObject:[NSNumber numberWithFloat:self.selectCoupon.couponAmount] forKey:@"couponAmount"];
        }
        [orderDic setObject:model.snapshotVersion forKey:@"snapshotVersion"];
        [orderDic setObject:[NSNumber numberWithFloat:model.price*model.goodNum] forKey:@"totalAmount"];
        [orderDic setObject:[NSNumber numberWithFloat:model.price] forKey:@"unitRent"];
        [orderDic setObject:[NSNumber numberWithFloat:model.price*model.goodNum] forKey:@"totalRent"];
        if (self.remark) {
            [orderDic setObject:self.remark forKey:@"remark"];
        }
        
        [orderArr addObject:orderDic];
    }
    NSString *goodsIDArr = [[NSString alloc]initWithData:[NSJSONSerialization dataWithJSONObject:orderArr options:0 error:nil] encoding:NSUTF8StringEncoding];
    
    NSMutableDictionary *param1 = [[NSMutableDictionary alloc] init];
    [param1 setValue:BWTUsertoken forKey:@"token"];
    [param1 setValue:[NSNumber numberWithInteger:BWTUserID] forKey:@"userId"];
    [param1 setValue:[NSNumber numberWithInteger:self.addressID] forKey:@"addressId"];
    [param1 setValue:goodsIDArr forKey:@"orderListJson"];

    @weakify(self);
    
    [AFNetworkTool postJSONWithUrl:ShoppingCart_creat_order parameters:param1 success:^(id responseObject) {
        @strongify(self);
        if (!self) return;
        BWTLog(@"%@", responseObject);
        if (responseObject) {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return 6 + self.cellModelArr.count;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 0) {
        ...
        UIImageView *bottomLine = [[UIImageView alloc] initWithFrame:CGRectMake(0, 130-3, kScreenWidth, 3)];
    ...
    if (indexPath.row > 0 && indexPath.row <= self.cellModelArr.count) {
        OrderGoodTVC *cell = [[OrderGoodTVC alloc] init];
        cell.orderCellModel = self.cellModelArr[indexPath.row - 1];
        
        return cell;
    }
    if (self.cellModelArr.count < indexPath.row && indexPath.row <=1 + self.cellModelArr.count) {
        UITableViewCell *cell = [[UITableViewCell alloc] init];
        cell.backgroundColor = BWTBackgroundGrayColor;
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
...
//        _couponArrowImg.hidden = self.isShowCouponUseLabel;
    ...
    if (indexPath.row == 5 + self.cellModelArr.count) {
        UITableViewCell *cell = [[UITableViewCell alloc] init];
        cell.backgroundColor = BWTBackgroundGrayColor;
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.cellModelArr.count + 1 < indexPath.row && indexPath.row <=4 + self.cellModelArr.count) {
        return 55;
    }
    if (indexPath.row == 5 + self.cellModelArr.count) {
        return 100;
