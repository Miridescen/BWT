...
@interface BWTChangPhoneVC ()<UITextFieldDelegate>
...
@implementation BWTChangPhoneVC
...
- (void)changeBtnClick:(UIButton *)btn{
    ...
    @weakify(self);
    [AFNetworkTool postJSONWithUrl:User_bound_phone parameters:param success:^(id responseObject) {
        @strongify(self);
        if (!self) return;
        
        [BWTUserTool changeUserPhone:self.phone];
...
- (void)addTopView{
    ...
    [loginBtn addTarget:self action:@selector(changeBtnClick:) forControlEvents:UIControlEventTouchUpInside];
